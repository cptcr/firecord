...
const { execSync } = require('child_process'); // For potential future use
const { setup } = require('./exec/setup'); // For potential future use
require('dotenv').config();
const ping = require("ping");
const os = require("os");

const loadCommands = require("./scripts/loadCommands"); // Assuming this file contains the loadCommands function
...
async function initialize() {
  ...
  if (!fs.existsSync(databaseFilePath) || !fs.existsSync(envFilePath)) {
    console.log(chalk.yellow('Setup required!. Running setup...'));
    setTimeout(async () => {
      console.clear();
      try {
        await setup();
