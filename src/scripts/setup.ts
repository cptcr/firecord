...
function appendEnvVariable(key: string, value: string) {
  ...
  const botDetails = await inquirer.prompt([
    {
      ...
      validate: (input: string) => input ? true : "Bot ID cannot be empty."
    },
    {
      type: "input",
      name: "token",
...
async function dbSetupConsole(name: string, inquirer: any) {
  ...
  const detailsDB = await inquirer.prompt([
    {
      ...
      validate: (input: string) => input ? true : "Hostname cannot be empty."
    },
    {
      type: "input",
      name: "dbFile",
      message: "Enter the database filename here (for example database.db):",
      validate: (input: string) => input ? true : "Database filename cannot be empty."
    },
    {
      type: "input",
      name: "dbPassword",
      message: "Enter the database password here:",
      validate: (input: string) => input ? true : "DB Password cannot be empty."
    }
  ]);
...
async function dbSetupWrite(hostname: string, dbFile: string, dbPassword: string) {
